...
public interface DepartmentRepository extends JpaRepository<Department, Long> {
    ...
    @Query(value = "SELECT COUNT(ds.id) FROM departments d "
            ...
            + "WHERE d.name = ?1 AND ds.degree_name = 'ASSISTANT'", nativeQuery = true)
    ...
    @Query(value = "SELECT COUNT(ds.id) FROM departments d "
            ...
            + "JOIN lectors l ON ld.lector_id = l.id "
            + "JOIN degrees ds ON l.degree_id = ds.id "
            + "WHERE d.name = ?1 AND ds.degree_name = 'ASSOCIATE_PROFESSOR'", nativeQuery = true)
    ...
    @Query(value = "SELECT COUNT(ds.id) FROM departments d "
            ...
            + "JOIN lectors l ON ld.lector_id = l.id "
            + "JOIN degrees ds ON l.degree_id = ds.id "
            + "WHERE d.name = ?1 AND ds.degree_name = 'PROFESSOR'", nativeQuery = true)
